...
function copyFile(srcDir, tarDir, cb) {
  ...
  // console.log(count, filesLen, folderLen)
...
module.exports = {
  ...
  // 拷贝模板
  copyTempToProject: (projectScaffold, projectName, cb) => {
    const proPath = path.resolve(CWD, projectName);
    const tempPath = path.resolve(CWD, projectScaffold);
    
    // 项目（仓库）的package.json
    const proJson = path.resolve(CWD, `${projectName}/package.json`);

    // 模板的package.json
    const tempJson =  path.resolve(CWD, `${projectScaffold}/package.json`);
